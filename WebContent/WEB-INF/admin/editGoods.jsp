...
<body>
    ...
    <form action="${ctx }/goods/updateGoods" id="form1" method="post">
    <ul class="forminfo">
    <li><label>商品类型</label> <div class="vocation">
    		<select class="select3" name="goodtype" id="goodtype">
				   <c:forEach var="type" items="${tList }">
        				<option value="${type.tid }"   <c:if test="${type.tid==goods.goodtype }"> selected="selected" </c:if>   />${type.tname }</option>
        			</c:forEach>
				   </select>
    </div>
    <input name="status" id="status" type="hidden" value="0" class="dfinput" /></li>
     <input name="gid" id="gid" type="hidden" value="${goods.gid }" class="dfinput" />
    <li><label>商品名称</label><input name="goodsname" id="goodsname" type="text" value="${goods.goodsname }" class="dfinput" /></li>
